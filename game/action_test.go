...
// ActionTestSuite is a suite of unit tests for actions.
...
func (suite *ActionTestSuite) SetupTest() {
	suite.from = player.New("playerFrom")
	suite.from.SetName(FromName)
	suite.to = player.New("playerTo")
	suite.to.SetName(ToName)
	suite.player3 = player.New("playerThree")
...
func (suite *ActionTestSuite) TestSorcererPositiveView() {
	...
	result := suite.game.NightAction(&FingerPoint{suite.from, suite.to})
	assert.Equal(suite.T(), fmt.Sprintf("%s %s", suite.to.GetIdentifier(), IsSeer), result.PlayerMessage)
	assert.Nil(suite.T(), result.Killed)
}
...
func (suite *ActionTestSuite) TestSorcererNegativeView() {
	...
	result := suite.game.NightAction(&FingerPoint{suite.from, suite.to})
	assert.Equal(suite.T(), fmt.Sprintf("%s %s", suite.to.GetIdentifier(), IsNotSeer), result.PlayerMessage)
	assert.Nil(suite.T(), result.Killed)
}
...
func (suite *ActionTestSuite) TestSeerPositiveView() {
	...
	result := suite.game.NightAction(&FingerPoint{suite.from, suite.to})
	assert.Equal(suite.T(), fmt.Sprintf("%s %s", suite.to.GetIdentifier(), IsWerewolf), result.PlayerMessage)
	assert.Nil(suite.T(), result.Killed)
}
...
func (suite *ActionTestSuite) TestSeerNegativeView() {
	...
	result := suite.game.NightAction(&FingerPoint{suite.from, suite.to})
	assert.Equal(suite.T(), fmt.Sprintf("%s %s", suite.to.GetIdentifier(), IsNotWerewolf), result.PlayerMessage)
	assert.Nil(suite.T(), result.Killed)
}
...
func (suite *ActionTestSuite) TestAuxSeerPositiveView() {
	...
	result := suite.game.NightAction(&FingerPoint{suite.from, suite.to})
	assert.Equal(suite.T(), fmt.Sprintf("%s %s", suite.to.GetIdentifier(), IsAuxEvil), result.PlayerMessage)
	assert.Nil(suite.T(), result.Killed)
}
...
func (suite *ActionTestSuite) TestAuxSeerNegativeView() {
	...
	result := suite.game.NightAction(&FingerPoint{suite.from, suite.to})
	assert.Equal(suite.T(), fmt.Sprintf("%s %s", suite.to.GetIdentifier(), IsNotAuxEvil), result.PlayerMessage)
	assert.Nil(suite.T(), result.Killed)
}
...
func (suite *ActionTestSuite) TestKnowsMax() {
	...
	result := suite.game.StartAction(suite.from)
	assert.Equal(suite.T(), fmt.Sprintf("%s %s", WolfListSingle, suite.to.GetIdentifier()), result.PlayerMessage)
}
